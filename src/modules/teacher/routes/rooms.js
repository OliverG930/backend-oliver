...
router.get('/rooms', seguridad(), async (req, res) => {
  const { usuario_id: usuarioId } = req.user
  const rooms = await controller.getRooms(usuarioId)
  return responses.success(req, res, rooms, 200)
})

router.get('/rooms/:id/students', seguridad(), async (req, res) => {
  const { id } = req.params

  const users = await controller.getUserRoom({ id: Number(id) })
  return responses.success(req, res, users, 200)
})
...
router.put('/room', seguridad(), isTeacher(), async (req, res) => {
  ...
  await controller.updateRoom(aula_id, body)
    ...
    .catch(() => responses.error(req, res, { message: 'imposible actualizar' }, 500))
